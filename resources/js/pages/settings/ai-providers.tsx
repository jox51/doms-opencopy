...
export default function AiProviders({ providers, availableProviders }: Props) {
    ...
    const filteredModels = (() => {
        ...
        const { models, imageModels } = selectedProviderConfig;
        if (data.supports_image && !data.supports_text && imageModels.length > 0) {
            return imageModels;
        }
        if (data.supports_text && !data.supports_image) {
            return models;
        }
                                ...
                                <div className="flex items-center gap-6">
                                    <div className="flex items-center space-x-2">
                                        <Switch
                                            ...
                                            onCheckedChange={(checked) => {
                                                const newData = { ...data, supports_text: checked };
                                                const config = selectedProviderConfig;
                                                if (config) {
                                                    const models = checked && !newData.supports_image
                                                        ? config.models
                                                        : !checked && newData.supports_image && config.imageModels.length > 0
                                                            ? config.imageModels
                                                            : [...config.models, ...config.imageModels];
                                                    if (models.length > 0 && !models.includes(data.model)) {
                                    ...
                                    <div className="flex items-center space-x-2">
                                        <Switch
                                            ...
                                            onCheckedChange={(checked) => {
                                                const newData = { ...data, supports_image: checked };
                                                const config = selectedProviderConfig;
                                                if (config) {
                                                    const models = checked && !newData.supports_text && config.imageModels.length > 0
                                                        ? config.imageModels
                                                        : !checked && newData.supports_text
                                                            ? config.models
                                                            : [...config.models, ...config.imageModels];
                                                    if (models.length > 0 && !models.includes(data.model)) {
                                                        newData.model = models[0];
